jQuery(function($) {
	...
	$("#login").submit(function(e){
		...
		socket.emit('login', {username: $("#username").val(), password: $("#password").val()}, function(data){
			if (data) {
				...
				$("#log").append("nick " + $("#username").val() + " (true)<br>");
			}else
			{
				$("#log").append("nick " + $("#username").val() + " (false)<br>");
			}
		});
	});
	...
	c.onclick = function(e){
	   console.log("move");
	    ...
	    socket.emit('move user', {x: e.x, y: e.y});
	};
	function renderSlots () {
		...
		for (var i = 0; i < data.length; i++) {
			...
			ctx.fillStyle = '#000';
			ctx.fillText(possRoot.player,possRoot.x+(52/2),possRoot.y+80);
